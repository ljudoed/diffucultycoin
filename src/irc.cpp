...
void ThreadIRCSeed()
{
    ...
    RenameThread("DifficultyCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DifficultyCoinTEST0\r");
            Send(hSocket, "WHO #DifficultyCoinTEST0\r");
        } else {
            // randomly join #DifficultyCoin00-#DifficultyCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #DifficultyCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DifficultyCoin%02d\r", channel_number).c_str());
        }

...
uint256 hashGenesisBlock("0x00000b44f87dd8fb25e26faad97c52d1e96741ffec84823471c218100b178973");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DifficultyCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DifficultyCoin Signed Message:\n";
...
{
    // DifficultyCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
   int64 nSubsidy =  0*COIN + (int64)round(GetDifficulty() * COIN);
        ...
        {
            nSubsidy = 0*COIN + (int64)round(GetDifficulty() * COIN);
        }
...
/*DGW 2 thank you, written by Evan Duffield */
...
{
    // DifficultyCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "DifficultyCoin to da Moooooooon!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1398003918;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 1025220;
        ...
        assert(block.hashMerkleRoot == uint256("0x7ac181851f94cbc3a5cae6bcfa88c8386fad3d707fd82af007646fd8066c61fe"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc0, 0xb8, 0xde }; // DifficultyCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DifficultyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DifficultyCoinMiner : ProcessBlock, block not accepted");
    }
